...
function load() {
    ...
    lab = lab.wait(function() {
        ...
        (function($) {
            ...
            window.Freemix.property.initializeFreemix();
...
(function() {
    if (!window.google) {
        var viewset = window.Freemix.profile["views"];
        for (var key in viewset) {
            for (var inx in viewset[key]) {
                if (viewset[key][inx].type==="piechart") {
                    lab =lab.script("http://www.google.com/jsapi").wait(function() {
                        google.load("visualization", "1", {packages:["piechart"], callback: load});
                    });
                    loaded_google=true;
                    return;
                }
