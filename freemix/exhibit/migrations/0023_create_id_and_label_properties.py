...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for exhibit in orm.Exhibit.objects.all():
            if exhibit.properties.filter(name="id").count() == 0:
                orm.ExhibitProperty(name="id", exhibit=exhibit, label="id", classname="ExhibitProperty").save()

            if exhibit.properties.filter(name="label").count() == 0:
                orm.ExhibitProperty(name="label", exhibit=exhibit, label="label", classname="ExhibitProperty").save()
