...
class ManyToManySearchInput(forms.SelectMultiple):
    ...
    def render(self, name, value, attrs=None):
        ...
        for val in value:
            ...
            ul += "<li id='%(pk)s'><div class='object_repr'>%(repr)s</div><div class='delete_from_m2m'><a style='cursor:pointer'><img src='%(admin_media_prefix)simg/admin/icon_deletelink.gif' /></a></div></li>" % {
            'admin_media_prefix': settings.ADMIN_MEDIA_PREFIX,
            'repr': self.to_string_function(obj),
            'pk': str(obj.pk)}
            select += '<option id="%(pk)s" value="%(pk)s" selected="selected">%(repr)s</option>' % {
            'admin_media_prefix': settings.ADMIN_MEDIA_PREFIX,
            'repr': self.to_string_function(obj),
            'pk': str(obj.pk)}
