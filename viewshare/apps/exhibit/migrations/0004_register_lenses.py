...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for exhibit in orm.Exhibit.objects.all():
            profile = json.loads(exhibit.profile)
            lenses = []
            views = profile["views"]["views"]
            for view in views:
                if view["type"] in ["list", "map", "scatterplot", "timeline"]:
                    id = str(uuid.uuid4())
                    lens = {
                        "properties": view["properties"],
                        "name": "%s lens"%view["name"],
                        "id": id,
                        "type": "list"
                    }

                    if "title" in view:
                        lens["title"] = view["title"]
                        del view["title"]

                    if "titleLink" in view:
                        lens["titleLink"] = view["titleLink"]
                        del view["titleLink"]
