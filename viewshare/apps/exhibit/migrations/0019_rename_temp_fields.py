...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'DraftExhibit.uuid'
        db.delete_column(u'exhibit_draftexhibit', 'uuid')

        # Rename field 'Exhibit.s' to 'Exhibit.slug'
        db.rename_column(u'exhibit_exhibit', 's', 'slug')

        # Rename field 'Exhibit.o' to 'Exhibit.owner'
        db.rename_column(u'exhibit_exhibit', 'o_id', 'owner_id')

        # Adding unique constraint on 'Exhibit', fields ['owner', 'is_draft', 'slug']
        db.create_unique(u'exhibit_exhibit', ['owner_id', 'is_draft', 'slug'])


    def backwards(self, orm):
        # Removing unique constraint on 'Exhibit', fields ['owner', 'is_draft', 'slug']
        db.delete_unique(u'exhibit_exhibit', ['owner_id', 'is_draft', 'slug'])

        # Adding field 'DraftExhibit.uuid'
        db.add_column(u'exhibit_draftexhibit', 'uuid',
                      self.gf('django.db.models.fields.CharField')(default='', max_length=36, blank=True),
                      keep_default=False)

        # Rename field 'Exhibit.owner'
        db.rename_column(u'exhibit_exhibit', 'owner_id', 'o_id')

        # Rename field 'Exhibit.slug'
        db.rename_column(u'exhibit_exhibit', 'slug', 's')
