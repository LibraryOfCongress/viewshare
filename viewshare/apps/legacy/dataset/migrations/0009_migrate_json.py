...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for ds in orm["dataset.Dataset"].objects.all():
            orm["dataset.DatasetProfile"](dataset =ds, data=ds.profile).save()

            orm["dataset.DatasetJSONFile"](dataset=ds, data=ds.data).save()

            orm["dataset.DatasetPropertiesCache"](dataset=ds, data=ds.properties_cache).save()


    def backwards(self, orm):
        for ds in orm["dataset.Dataset"].objects.all():
            try:
                ds.data = orm["dataset.DatasetJSONFile"].get(dataset=ds).data
            except:
                pass
            try:

                ds.profile = orm["dataset.DatasetProfile"].get(dataset=ds).data
            except:
                pass
            try:
                ds.properties_cache = orm["dataset.DatasetPropertiesCache"].get(dataset=ds).data
            except:
                pass
            ds.save()
